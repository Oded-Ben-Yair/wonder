...
// SPA support - serve index.html for client-side routes
// BUT properly handle all other cases
app.get('*', (req, res, next) => {
  ...
  // Skip file requests (assets, css, js, images, etc) - let them 404 naturally
  if (req.path.includes('.')) {
    return next();
  }

  // For all other paths (client-side routes), serve the SPA
  res.sendFile(path.join(__dirname, 'public', 'index.html'));
});

// 404 handler for everything that wasn't handled
app.use((req, res) => {
  res.status(404).json({ error: 'Not found', path: req.path });
});

// Global error handler - CRITICAL for proper error responses
app.use((err, req, res, next) => {
  ...
  // Never send HTML error pages - always send JSON
