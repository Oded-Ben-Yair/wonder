...
async function loadData() {
  try {
    ...
    const namesPath = path.join(__dirname, 'packages/gateway/src/data/nurse_names.json');
    const namesContent = await fs.readFile(namesPath, 'utf-8');
    nameMapping = JSON.parse(namesContent);

    // Load Hebrew search index
    const indexPath = path.join(__dirname, 'packages/gateway/src/data/hebrew_search_index.json');
    const indexContent = await fs.readFile(indexPath, 'utf-8');
    hebrewIndex = JSON.parse(indexContent);

    // Load nurses data
    const nursesPath = path.join(__dirname, 'packages/gateway/src/data/nurses.json');
