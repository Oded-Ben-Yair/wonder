...
start();
// Simplified engine loading for Azure
async function loadEnginesSimple() {
  try {
    const adapterPath = path.join(__dirname, '..', 'engine-basic', 'adapter.js');
    const adapter = await import(`file://${adapterPath}`);
    ...
    logger.info({ count: engines.size }, 'Engine loaded (Azure mode)');
  } catch (error) {
    logger.error({ error: error.message }, 'Failed to load engine');
  }
}
