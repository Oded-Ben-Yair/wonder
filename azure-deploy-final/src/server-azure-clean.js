...
// Azure-specific: Direct import of scoring engine
import { basicMatch } from '../engine-basic/src/lib/basic-azure.js';
...
// Azure-specific: Direct engine registration (no scanning)
async function loadEngines() {
  try {
    // Directly register engine-basic with imported basicMatch function
    engines.set('engine-basic', {
      name: 'engine-basic',
      match: async (query, nurses) => {
        // basicMatch returns {results, _locationCount, _serviceCount, _availableCount}
        const result = basicMatch(query, nurses);
        return result;
      },
      health: async () => ({
        ok: true,
        engine: 'engine-basic',
        message: 'Ready (Azure direct mode)'
      })
    });

    logger.info({ count: engines.size, mode: 'Azure direct import' }, 'Engine loaded successfully');
  } catch (error) {
    logger.error({ error: error.message, stack: error.stack }, 'Failed to load engine');
    throw error;
  }
}
...
const corsOptions = {
  origin: function (origin, callback) {
    ...
    const allowedPatterns = [
      ...
      /^https?:\/\/.*\.onrender\.com$/,
      /^https?:\/\/.*\.railway\.app$/,
      /^https?:\/\/.*\.azurewebsites\.net$/  // Azure App Service domains
    ];
...
// Simplified static file serving (no complex setHeaders that could crash)
...
app.use((err, req, res, next) => {
  ...
  res.status(500).json({
    error: 'Server error',
    message: process.env.DEBUG ? err.message : 'An error occurred'
  });
});
